...
class PredictionControls:
    ...
    def render(self) -> None:
        ...
        st.divider()
        st.subheader(_("Step 10: Kinetic Predictions"))

        ae_results = SessionManager.get_active_ae_result()
        if ae_results is None:
            st.info(_("Complete Step 7 (Activation Energy) first to enable predictions."))
            return

        tab_mf, tab_mb = st.tabs([_("Model-free Prediction"), _("Model-based Isothermal")])
    ...
    def _render_modelfree_controls(self) -> None:
        ...
        mode = st.radio(
            _("Temperature program"),
            options=["Isothermal", "Linear heating"],
            format_func=lambda x: _("Isothermal") if x == "Isothermal" else _("Linear heating"),
            key="pred_mode_radio",
            help=_(
        ...
        with col1:
            ...
            st.number_input(
                ...
                help=_("Simulation runs until this conversion value is reached."),
            )

        with col2:
            st.markdown(_("**Time search bounds (min)**"))
        ...
        st.write("")
        if st.button(_("Run Model-free Prediction"), type="primary", key="pred_mf_run_btn"):
    ...
    def _render_modelbased_controls(self) -> None:
        ...
        with col1:
            st.number_input(
                ...
                step=5.0,
                key="pred_mb_iso_T",
                help=_("Constant temperature at which to predict conversion vs time."),
            )

            if b_num is not None:
                beta_options = {i: _("β = {beta:.2f} K/min").format(beta=b) for i, b in enumerate(b_num)}
                st.selectbox(
                    _("Reference heating rate"),
                    options=list(beta_options.keys()),
                    format_func=lambda x: beta_options[x],
                    key="pred_mb_col",
                    help=_("Heating rate whose temperature profile is used for the integral."),
                )

        with col2:
            st.write("")

        if st.button(_("Run Model-based Prediction"), type="primary", key="pred_mb_run_btn"):
